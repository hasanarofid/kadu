  ...
  <div class="min-h-screen bg-slate-950 text-slate-100 font-sans selection:bg-indigo-500 selection:text-white flex flex-col relative">
    ...
    <header class="lg:hidden bg-slate-900/90 backdrop-blur-xl border-b border-slate-800 h-16 px-4 flex items-center justify-between sticky top-0 z-40">
      ...
      <div class="flex items-center gap-3">
        <div class="flex items-center gap-1.5 px-2.5 py-1 bg-amber-500/10 border border-amber-500/30 rounded-full text-amber-300 text-xxs font-bold">
    ...
    <div class="flex-1 flex overflow-hidden">
      ...
      >
        <div class="space-y-6 p-6">
          ...
          <!-- User Card with Token Counter Widget -->
          <div class="p-4 rounded-2xl bg-slate-950 border border-slate-800/80 space-y-3">
            <div class="flex items-center gap-3">
              <div class="w-10 h-10 rounded-xl bg-gradient-to-tr from-indigo-600 to-violet-600 text-white font-extrabold flex items-center justify-center text-base shadow shrink-0">
            ...
            <!-- Token Counter Box -->
            <div class="p-3 bg-slate-900 border border-amber-500/30 rounded-xl flex items-center justify-between">
              <div>
                <span class="text-xxs font-bold text-slate-400 uppercase tracking-wider block">KUOTA TOKEN RPP</span>
                <span class="text-sm font-black text-amber-400">{{ userTokens }} Token</span>
              </div>
              <Link :href="route('tokens.index')" class="p-2 bg-amber-500/10 text-amber-300 rounded-lg hover:bg-amber-500/20 text-xxs font-bold border border-amber-500/30">
                + Beli
              </Link>
            </div>
          </div>
          ...
          <nav class="space-y-1.5 pt-2">
            <span class="text-xxs font-extrabold text-slate-400 uppercase tracking-widest block px-3 mb-2">MENU DASHBOARD</span>
